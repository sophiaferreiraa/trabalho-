...
export default function AgendaScreen() {
  ...
  const abrirNovo = () => {
    ...
    setCorSelecionada('#28a745');
    setModalVisivel(true);
  };

  const abrirEdicao = (t: Tarefa) => {
    setTarefaEmEdicao(t); 
    setTituloTarefa(t.titulo); 
    setHorarioTarefa(t.horario); 
    setMetaSelecionada(t.meta_id);
    setCorSelecionada(t.cor);
    setModalVisivel(true);
  };
    ...
    try {
      if (tarefaEmEdicao) {
        ...
        await axios.post(`${API_URL}/tarefas`, {
          ...
          titulo: tituloTarefa, 
          horario: horarioTarefa,
          data: formatDateLocal(diaSelecionado), 
          cor: corSelecionada,
          meta_id: metaSelecionada
        });
      }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.container}>
        ...
        <View style={styles.header}>
          <View>
            ...
            {/* Explicação basiquinha adicionada aqui */}
          ...
          <View style={styles.modalBox}>
            ...
            <View style={styles.priorityContainer}>
              {PRIORIDADES.map((p) => (
                <TouchableOpacity
                  key={p.value}
                  style={[ styles.priorityBtn, { backgroundColor: p.displayColor, opacity: corSelecionada === p.value ? 1 : 0.3 } ]}
                  onPress={() => setCorSelecionada(p.value)}
                >
      ...
      <TutorialOverlay 
        visible={passoAtual === 1}
        titulo="2. Organize seu Dia"
        texto="Agora você pode definir a prioridade das suas tarefas. Elas serão ordenadas automaticamente!"
        botaoTexto="Ir para Gráficos 👉"
