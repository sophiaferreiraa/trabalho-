...
class ProdutoDAO {
    ...
    // Filtrar produtos por categoria (ID do Product_Types)
    ...
    // Cadastrar produto e vincular à categoria
    public function cadastrar(ProdutoModel $produto) {
        try {
            $this->pdo->beginTransaction();

            // 1. Inserir na tabela Products
            ...
            // 2. Vincular Categoria na tabela N:N (Product_Product_Types)
    ...
    // Excluir produto (O CASCADE do banco remove os relacionamentos)
    ...
    // Buscar categorias para preencher o <select> e o filtro
    public function listarCategorias() {
        $stmt = $this->pdo->query("SELECT * FROM Product_Types ORDER BY nome ASC");
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    // Função auxiliar para converter array do banco em Objetos
