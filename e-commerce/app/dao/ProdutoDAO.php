...
class ProdutoDAO {
    ...
    private function mapearLista($rows) {
        ...
        foreach ($rows as $row) {
            ...
            $p->setBrandId($row['brand_id']);
            $p->setBrandName($row['brand_name']);

            $p->setHairTypeId($row['hair_type_id']);
            $p->setHairType($row['hair_type']);

            $p->setCurlPatternId($row['curl_pattern_id']);
            $p->setCurlPattern($row['curl_pattern']);

            $p->setProductTypeId($row['product_type_id']);
            $p->setProductType($row['product_type']);

            $p->setTreatmentTypeId($row['treatment_type_id']);
            $p->setTreatmentType($row['treatment_type']);
