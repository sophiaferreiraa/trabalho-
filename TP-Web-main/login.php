...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password_raw)) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT user_id, senha, nome_completo FROM Users WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            ...
            if ($user) {
                if (password_verify($password_raw, $user['senha'])) {
                    ...
                    $feedback = "Login realizado com sucesso! Bem-vindo(a), " . htmlspecialchars($user['nome_completo']) . ".";
                    $feedback_type = 'success';
                    $login_successful = true;
                    
                } else {
                    $feedback = "Email ou palavra-passe incorreta.";
                    $feedback_type = 'error';
                }
            } else {
                $feedback = "Email ou palavra-passe incorreta.";
