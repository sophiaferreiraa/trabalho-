...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password_raw)) {
        ...
        try {
            // CORREÇÃO 1: Alterado 'user_id' para 'id' para combinar com seu banco de dados
            $stmt = $pdo->prepare("SELECT id, senha, nome_completo FROM Users WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();

            if ($user) {
                // A senha no banco ESTÁ CERTA (criptografada). Esta função compara a senha digitada com a do banco.
                if (password_verify($password_raw, $user['senha'])) {
                    
                    // CORREÇÃO 2: Usando o ID correto vindo do banco
                    $_SESSION['user_id'] = $user['id'];
                    $_SESSION['nome_completo'] = $user['nome_completo'];
                    
                    // CORREÇÃO 3: Redirecionamento para o index.php
            ...
            // Se quiser ver o erro exato na tela, descomente a linha abaixo:
