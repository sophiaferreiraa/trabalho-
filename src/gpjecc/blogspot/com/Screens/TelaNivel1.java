...
public class TelaNivel1 implements Screen, PlayerObserver {
    ...
    // private Ladders ladders; // Não é mais necessário para desenhar
    private Rectangle victoryZone; // Apenas para debug local, se necessário
    ...
    private boolean gameWon = false; // Flag local para parar de processar
    ...
    public void show() {
        ...
        blocks = new Blocks(1); // Cliente desenha os blocos
    ...
    public void render(float delta) {
        ...
        // Se o jogo acabou (vitória ou derrota), não faz nada
        if (gameWon || gameOver) {
             gameOverTimer += delta;
             if (gameOverTimer >= 5f && gameOver) {
                 // Se perdemos, saímos
                 Gdx.app.exit();
             }
             // Se ganhamos (gameWon), a TelaVitoria será chamada
             return; 
        }

        // 1) Coletar input do jogador local
        PlayerInput input = new PlayerInput();
        if(player.isAlive()) { // Só envia input se estiver vivo
        ...
        // --- LÓGICA DE VITÓRIA LOCAL REMOVIDA ---
    ...
    private void applySnapshotOnRenderThread(GameSnapshot snap) {
        ...
        if ("VICTORY".equals(snap.gameState) && !gameWon) {
            gameWon = true; // Seta a flag local
            Gdx.app.log("TelaNivel1", "Servidor reportou VICTÓRIA! Indo para a tela de vitória...");
            jogo.goToWinScreen(2); // Vai para a tela de transição
            return; // Para de processar este snapshot
        }
        // ---------------------------------
