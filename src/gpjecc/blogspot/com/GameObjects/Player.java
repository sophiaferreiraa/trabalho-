...
public class Player extends GameObject {
    ...
    public Player() {
        ...
        this.health = 5f; // <-- HP INICIAL 5.0
    }

    // --- CORREÇÃO DO CRASH ESTÁ AQUI ---
    // Construtor "leve" (Jogador Remoto / Fantasma)
    protected Player(boolean noLoad) {
        // CHAMA O CONSTRUTOR DO GAMEOBJECT PARA CARREGAR ASSETS
        super(new Vector2(0, 0), new Vector2(23, 45), "spriteTartaruga");

        this.isRemote = true; // Define como remoto
        
        // Carrega as animações (NECESSÁRIO PARA EVITAR O NULLPOINTER)
        this.animationFrames = AssetsManager.getInstance().getFrames("spriteTartaruga", new Vector2(32, 64));
        this.animation = AssetsManager.getInstance().getAnimation(animationFrames, 0, 0.15f);
        this.currentFrame = AssetsManager.getInstance().getCurrentFrame(animation); // <-- LINHA CRÍTICA
    ...
    // Dano local (Inimigos) - O seu log mostrou que isso já estava funcionando!
    ...
    public void update(float delta) {
        ...
        if (isRemote) { 
            ...
            // Adiciona uma verificação de segurança (embora o construtor deva ter corrigido)
            ...
            return; // NÃO processa input local, gravidade ou colisões
        }
